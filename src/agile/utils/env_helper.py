...
class EnvHelper:
    ...
    def _load_env_files(self, env_file_path: str | Path = None, override: bool = False):
        ...
        """
        env_files: list[Path] = []
        # 如果提供了 env_file_path 参数，根据其是否以 .env 结尾来决定加载方式
        if env_file_path:
            # 如果不是 .env 结尾的文件路径，则将其作为主环境文件加载，并根据 env_mode 添加环境专属文件和通用 .env 文件到加载列表，作为次环境文件加载
            ...
            # 如果 env_file_path 是 .env 文件，则根据 env_mode 添加环境专属文件和通用 .env 文件到加载列表，作为次环境文件加载
            env_files.append(Path(env_file_path))
        # 未提供 env_file_path 参数，根据 env_mode 添加环境专属文件和通用 .env 文件到加载列表，按优先级加载
        else:
            root_path = pyrootutils.find_root()
            if self.env_mode:
                env_files.append(root_path.joinpath(f".env.{self.env_mode}"))
            env_files.append(root_path.joinpath(".env"))
